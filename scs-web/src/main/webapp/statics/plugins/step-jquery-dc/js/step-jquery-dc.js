...
Step_Tool_dc.prototype = {
    ...
    createStepArray: function (currStep, stepListJson) {
        ...
        for (var i = 0; i < stepListJson.length; i++) {
            var Step_Obj = new Step(this.currStep, stepListJson[i].StepNum, stepListJson[i].text, stepListJson[i].StepText, stepListJson.length);
...
var Step = function (currStep, StepNum, text, StepText, totalCount) {
    this.currStep = currStep,
...
Step.prototype = {
    createStepHtml: function () {
        var stepHtml = "\<span\>" + this.text + "\</span\>";
        stepHtml = stepHtml + "\<a  data-value=\"" + this.StepNum + "\" data-text=\"" + this.StepText + "\" \>" + this.StepText + "\</a\>";
