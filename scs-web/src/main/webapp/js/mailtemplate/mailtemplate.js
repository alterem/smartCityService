...
var vm = new Vue({
	...
	methods: {
		...
		saveOrUpdate: function (event) {
			var url = vm.mailtemplate.id == null ? "../mailtemplate/save" : "../mailtemplate/update";
			$.ajax({
				type: "POST",
			    url: url,
			    data: JSON.stringify(vm.mailtemplate),
			    success: function(r){
			    	if(r.code === 0){
		...
		del: function (id) {
			confirm('确定要删除选中的记录？', function(){
		    	$.get("../mailtemplate/delete/"+id, function(r){
					if(r.code == 0){
		...
		getInfo: function(id){
			$.get("../mailtemplate/info/"+id, function(r){
                vm.mailtemplate = r.mailtemplate;
            });
		},
