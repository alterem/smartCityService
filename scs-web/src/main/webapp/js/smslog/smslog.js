...
var vm = new Vue({
	...
	data:{
		...
		upd: false,
		title: null,
		smslog: {status:1, content:"",receiver:'',receiverName:''}
	},
	methods: {
		query: function () {
			vm.reload();
		...
		add: function(){
			...
			vm.save = true,
			vm.title = "新增";
			vm.smslog = {status:1, content:"",receiver:'',receiverName:''};
			...
			$.get("../smstemplate/getSendTemplate", function(r){
				if(r.code != 0){
					alert(r.msg);
				} else {
					rdata = r.data;
					$('#tid').combobox({
						...
						onChange:function(newValue, oldValue){
							// 再请求一次做进一步处理
							$.get("../smstemplate/info2/" + newValue, function(r2){
								if(r2.code != 0){
									alert(r2.msg);
								} else {
									rdata2 = r2.data;
									//$("[name='content']").text(rdata2.smstemplate.content);
									vm.smslog.content = rdata2.smstemplate.content;
									temp = $("#par");
		...
		deptTree : function() {
			layer.open({
				...
				btn1 : function(index) {
					...
					vm.smslog.receiver = receiver.toString();
					vm.smslog.receiverName = receiverName.toString();
					layer.close(index);
				}
			});
		},
		...
		saveOrUpdate: function (event) {
			...
			smslog = $('.form-horizontal').form2json();
			$.ajax({
				type: "POST",
			    url: url,
			    data: JSON.stringify(smslog),
			    success: function(r){
			    	if(r.code === 0){
		...
		del: function (id) {
			confirm('确定要删除选中的记录？', function(){
		    	$.get("../smslog/delete/"+id, function(r){
					if(r.code == 0){
		...
		getInfo: function(id){
			$.get("../smslog/info/"+id, function(r){
                for(var name in r.smslog) {
					if(name == 'status'){
						$( ".form-horizontal" ).find( "[name='" + name + "']" ).text(r.smslog[ name ] == '1' ? '成功' : '失败' );
					} else {
						$( ".form-horizontal" ).find( "[name='" + name + "']" ).text(r.smslog[ name ] );
					}
				}
            });
		},
