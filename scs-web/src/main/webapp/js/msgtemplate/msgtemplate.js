...
var vm = new Vue({
	...
	methods: {
		...
		saveOrUpdate: function (event) {
			var url = vm.msgtemplate.id == null ? "../msgtemplate/save" : "../msgtemplate/update";
			$.ajax({
				type: "POST",
			    url: url,
			    data: JSON.stringify(vm.msgtemplate),
			    success: function(r){
			    	if(r.code === 0){
		...
		del: function (id) {
			confirm('确定要删除选中的记录？', function(){
		    	$.get("../msgtemplate/delete/"+id, function(r){
					if(r.code == 0){
		...
		getInfo: function(id){
			$.get("../msgtemplate/info/"+id, function(r){
                vm.msgtemplate = r.msgtemplate;
            });
		},
