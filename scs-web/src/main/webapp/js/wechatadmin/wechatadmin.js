...
var vm = new Vue({
	...
	methods: {
		...
		saveOrUpdate: function (event) {
			var url = vm.wechatadmin.id == null ? "../wechatadmin/save" : "../wechatadmin/update";
			$.ajax({
				type: "POST",
			    url: url,
			    data: JSON.stringify(vm.wechatadmin),
			    success: function(r){
			    	if(r.code === 0){
		...
		del: function (id) {
			confirm('确定要删除选中的记录？', function(){
		    	$.get("../wechatadmin/delete/"+id, function(r){
					if(r.code == 0){
		...
		getInfo: function(id){
			$.get("../wechatadmin/info/"+id, function(r){
                vm.wechatadmin = r.wechatadmin;
                $("[name='headimg']").attr("src", r.wechatadmin.headimgurl);
            });
		},
