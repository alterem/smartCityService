    ...
    $("#jqGrid").jqGrid({
        ...
        colModel: [			
			...
			{ label: '类型', name: 'type', width: 80 , formatter: function(value, options, row){
				...
				{ label: '来源', name: 'source', width: 80 , formatter: function(value, options, row){
				return $.isPlainObject(value) ? value.value : value;
			}}, 
				...
				{ label: '最近的地点', name: 'adds', width: 80 , formatter: function(value, options, row){
				return $.isPlainObject(value) ? value.value : value;
			}}, 
				...
				{ label: '案件编号', name: 'caseno', width: 80 , formatter: function(value, options, row){
				return $.isPlainObject(value) ? value.value : value;
			}}, 
...
var vm = new Vue({
	...
	methods: {
		...
		saveOrUpdate: function (event) {
			var url = vm.brknews.id == null ? "../brknews/save" : "../brknews/update";
			$.ajax({
				type: "POST",
			    url: url,
			    data: JSON.stringify(vm.brknews),
			    success: function(r){
			    	if(r.code === 0){
		...
		del: function (id) {
			confirm('确定要删除选中的记录？', function(){
		    	$.get("../brknews/delete/"+id, function(r){
					if(r.code == 0){
		...
		getInfo: function(id){
			$.get("../brknews/info/"+id, function(r){
                vm.brknews = r.brknews;
            });
		},
